...
@Component({
  selector: 'app-monthly-presences',
  standalone: true,
  imports: [
    CommonModule,
  ],
  templateUrl: './monthly-presences.component.html',
  styleUrl: './monthly-presences.component.css'
})


export class MonthlyPresencesComponent implements OnInit {
  ...
  getMonthlyPresences(month: number, year: number): void {
    this.presenceService.getMonthlyPresenceByDepartement(month, year, parseInt(this.departementId, 10)).subscribe(
      (response) => {
